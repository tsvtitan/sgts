...
begin
  ...
  if Assigned(Form) then begin
    ...
    Form.GroupBoxMeasureType.Enabled:=FEnabledFilterByMeasureType;
    Form.EditMeasureType.Enabled:=FEnabledFilterByMeasureType;
    Form.ButtonMeasureType.Enabled:=FEnabledFilterByMeasureType;
...
begin
  ...
  case Key of
    VK_DELETE, VK_BACK: begin
      if (Shift=[]) and (Edit.SelLength=Length(Edit.Text)) then begin
        Edit.Text:='';
        FMeasureTypeId:=Null;
      end;
    end;
...
begin
  ...
  try
    if Iface.SelectVisible('MEASURE_TYPE_ID',FMeasureTypeId,Buffer) then begin
