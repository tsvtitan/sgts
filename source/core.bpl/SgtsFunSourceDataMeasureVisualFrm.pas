  ...
  begin
    ...
    if FDataSetPoints.Active and
      ...
      with FDataSet do begin
        ...
        FSelectDefs.Add('POINT_NAME','Измерительная точка',50);
        FSelectDefs.Add('DATE_OBSERVATION','Дата измерения',70);
        FSelectDefs.Add('OBJECT_PATHS','Расположение объекта',80);
        FSelectDefs.Add('COORDINATE_Z','Отметка ИТ',50);
        FSelectDefs.Add('DEFECT_VIEW_NAME','Вид дефекта',150);
        FSelectDefs.Add('INCLINATION','Привязка',150);
        FSelectDefs.Add('WIDTH_DEFECT','Ширина дефекта',50);
        FSelectDefs.Add('HEIGHT_DEFECT','Выстоа дефекта',50);
        FSelectDefs.Add('LENGTH_DEFECT','Длина дефекта',50);
        ...
        FSelectDefs.AddDrawCheck('IS_CONFIRM_EX','Утверждено','IS_CONFIRM',30);
      ...
      with FDataSetDocs do begin
        ...
        FSelectDefsDocs.Add('DATE_DOC','Дата',70);
        FSelectDefsDocs.Add('NAME','Наименование',150);
        FSelectDefsDocs.Add('DESCRIPTION','Описание',200);
  ...
  if CanRefresh then begin
    ...
    try
      ...
      if CreateData then begin
        ...
        FGrid.Hint:='Исходные данные: '+MeasureTypeName+'  (осмотр)';
