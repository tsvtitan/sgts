...
begin
  ...
  try
    ...
    if DS.Active and not DS.IsEmpty then
      Result:=Format('%s. %s. ÈÒ %s מעל.%s %s',[DS.FieldByName('MEASURE_TYPE_NAME').AsString,
...
begin
  ...
  try
    ...
    AIface.EnabledFilterByMeasureType:=false;
  ...
  try
    ...
    if AIface.SelectByUnionType('UNION_ID;UNION_TYPE',VarArrayOf([APointId,utPoint]),
      ...
      if DS.Active and not DS.IsEmpty then begin
        ...
        Point.Caption:=Iface.GetPointCaption(AName,APointId,Point.RouteId);
        FillPoints;
        ListBoxPoints.ItemIndex:=Index;
        ListBoxPoints.Selected[Index]:=true; 
      end;
    end;
  finally
    DS.Free;
