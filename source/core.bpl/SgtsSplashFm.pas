...
begin
  ...
  if Assigned(ACoreIntf) then begin
    FileSplash:=ACoreIntf.Config.Read(ACoreIntf.Name,SConfigParamSplash,'');
    if not FileExists(FileSplash) then
      FileSplash:=ExtractFilePath(ACoreIntf.CmdLine.FileName)+ExtractFileName(FileSplash);

    ImageLoaded:=false;
    if FileExists(FileSplash) then begin
      try
        ImageSplash.Picture.LoadFromFile(FileSplash);
        ImageLoaded:=true;
      except
      end;
    end;
    LabelCaption.Visible:=not ImageLoaded;
  end;
end;
