...
type
  ...
  private
    ...
    function GetFirstHistorySelected(var Index: Integer): TSgtsBaseGraphHistory;
...
begin
  ...
  try
    History:=GetFirstHistorySelected(Index);
    if Assigned(History) then begin
      ...
      if AIface.Select(APeriodType,ADateBegin,ADateEnd,ACycleBegin,ACycleEnd) then begin
        ...
        History.DateEnd:=ADateEnd;
        History.CycleBegin:=ACycleBegin;
        History.CycleEnd:=ACycleEnd;
        FillHistories;
        ListBoxHistory.ItemIndex:=Index;
        ListBoxHistory.Selected[Index]:=true;
      end;
    end;
  finally
    AIface.Free;
...
begin
  History:=GetFirstHistorySelected(Index);
  MenuItemHistoryChange.Enabled:=Assigned(History);
  MenuItemHistoryDelete.Enabled:=Assigned(History);
  MenuItemHistoryClear.Enabled:=ListBoxHistory.Items.Count>0;
end;

function TSgtsBaseGraphPeriodRefreshForm.GetFirstHistorySelected(var Index: Integer): TSgtsBaseGraphHistory;
var
  i: Integer;
begin
  Result:=nil;
