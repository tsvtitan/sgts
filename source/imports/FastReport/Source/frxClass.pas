...
type
  ...
  // by TSV
  TfrxDesignReportAction=(draBeforeCreate,draBeforeShowModal,draAfterShowModal,draAfterFree);
  TfrxDesignReportCallbackProc=procedure (Report: TfrxReport; Action: TfrxDesignReportAction) of object;
...
{ Report }
  ...
  public
    ...
    // By TSV
    procedure DesignReport(Modal: Boolean = True; MDIChild: Boolean = False; AProc: TfrxDesignReportCallbackProc=nil); overload; stdcall;
...
// by TSV
procedure TfrxReport.DesignReport(Modal: Boolean = True; MDIChild: Boolean = False; AProc: TfrxDesignReportCallbackProc=nil);
var
  l: TList;
  i: Integer;
  c: TfrxComponent;
...
{$IFDEF FR_RUN_DESIGNER}
  ...
  begin
    ...
    begin
      if Assigned(AProc) then
        AProc(Self,draBeforeShowModal);
      FDesigner.ShowModal;
      if Assigned(AProc) then
        AProc(Self,draAfterShowModal);
      FDesigner.Free;
      if Assigned(AProc) then
        AProc(Self,draAfterFree);
