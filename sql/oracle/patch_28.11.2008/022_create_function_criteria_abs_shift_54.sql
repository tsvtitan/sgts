/* Создание функции расчета критерия абсолютного горизонтального перемещения гребня. Секция №54 */

CREATE OR REPLACE FUNCTION CRITERIA_ABS_SHIFT_54
(
...
AS
  ...
  ACYCLE_MAX INTEGER;
  FLAG INTEGER;
  APARAM_ID INTEGER:=17165 /* Смещ. от верт. по X с нач. набл. */;
  AMEASURE_TYPE_ID INTEGER:=4621 /* Отвесы в теле плотины */;
  AROUTE_ID INTEGER:=4660 /* Маршрут отвесов в теле плотины */;
  AOBJECT_PATHS VARCHAR2(100):='Плотина\Тело плотины\Секция ';
  ASECTIONS VARCHAR2(250):='54';
  ADIVIDER VARCHAR2(1):=',';
BEGIN
  SELECT FIRST_MIN_VALUE, FIRST_MAX_VALUE, SECOND_MIN_VALUE, SECOND_MAX_VALUE, 
    ...
    INTO FIRST_MIN_VALUE, FIRST_MAX_VALUE, SECOND_MIN_VALUE, SECOND_MAX_VALUE, 
         FIRST_MODULO, SECOND_MODULO 
	FROM CRITERIAS 
   WHERE CRITERIA_ID=CRITERIA_ABS_SHIFT_54.CRITERIA_ID;
