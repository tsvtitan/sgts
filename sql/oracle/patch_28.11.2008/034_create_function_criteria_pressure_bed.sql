/* Создание функции расчета критерия приведенного напора в основании плотины. Створные пьезометры. Русловые секции */

CREATE OR REPLACE FUNCTION CRITERIA_PRESSURE_BED
(
...
AS
  ...
  ACYCLE_MAX INTEGER;
  FLAG INTEGER;
  APARAM_ID INTEGER:=2963 /* Приведенный напор */;
  AMEASURE_TYPE_ID INTEGER:=2562 /* Створные пьезометры */;
  AROUTE_ID INTEGER:=2581 /* Маршрут створных */;
  AOBJECT_PATHS VARCHAR2(100):='Плотина\Тело плотины\Секция ';
  ASECTIONS VARCHAR2(250):='8,10,52,32,Б,А,Г,1,2,3,4,5,6,60,61,62,63,64,65,66,67,68,69,70';
  ADIVIDER VARCHAR2(1):=',';
BEGIN
  SELECT FIRST_MIN_VALUE, FIRST_MAX_VALUE, SECOND_MIN_VALUE, SECOND_MAX_VALUE, 
    ...
    INTO FIRST_MIN_VALUE, FIRST_MAX_VALUE, SECOND_MIN_VALUE, SECOND_MAX_VALUE, 
         FIRST_MODULO, SECOND_MODULO 
	FROM CRITERIAS 
   WHERE CRITERIA_ID=CRITERIA_PRESSURE_BED.CRITERIA_ID;
    ...
    IF (ADATE_BEGIN IS NULL) THEN
	  SELECT MIN(DATE_OBSERVATION) INTO ADATE_BEGIN
	   ...
	   WHERE PARAM_ID=APARAM_ID
		 ...
		 AND POINT_ID IN (SELECT POINT_ID FROM TABLE(CAST(GET_POINTS_OUT_SECTIONS(AROUTE_ID,ASECTIONS,AOBJECT_PATHS,ADIVIDER)AS POINT_TABLE)));
	END IF;
	...
	IF (ADATE_BEGIN IS NOT NULL) AND (ADATE_END IS NOT NULL) THEN
	  ...
	  FOR INC IN (SELECT JO.POINT_ID, JO.DATE_OBSERVATION, JO.CYCLE_ID, JO.VALUE
                   ...
                   WHERE JO.CYCLE_ID=C.CYCLE_ID
                     ...
                     AND JO.POINT_ID IN (SELECT POINT_ID FROM TABLE(CAST(GET_POINTS_OUT_SECTIONS(AROUTE_ID,ASECTIONS,AOBJECT_PATHS,ADIVIDER)AS POINT_TABLE)))
