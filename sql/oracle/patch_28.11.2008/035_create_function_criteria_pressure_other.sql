/* Создание функции расчета критерия приведенного напора в основании плотины. Створные пьезометры. Секции №8,10,52 */

CREATE OR REPLACE FUNCTION CRITERIA_PRESSURE_OTHER
(
...
AS
  ...
  ASECTIONS VARCHAR2(250):='8,10,52';
  ADIVIDER VARCHAR2(1):=',';
BEGIN
  SELECT FIRST_MIN_VALUE, FIRST_MAX_VALUE, SECOND_MIN_VALUE, SECOND_MAX_VALUE, 
    ...
    INTO FIRST_MIN_VALUE, FIRST_MAX_VALUE, SECOND_MIN_VALUE, SECOND_MAX_VALUE, 
         FIRST_MODULO, SECOND_MODULO 
	FROM CRITERIAS 
   WHERE CRITERIA_ID=CRITERIA_PRESSURE_OTHER.CRITERIA_ID;
    ...
    IF (ADATE_BEGIN IS NULL) THEN
	  SELECT MIN(DATE_OBSERVATION) INTO ADATE_BEGIN
	   ...
	   WHERE PARAM_ID=APARAM_ID
	     AND MEASURE_TYPE_ID=MEASURE_TYPE_ID
		 AND POINT_ID IN (SELECT POINT_ID FROM TABLE(CAST(GET_POINTS_IN_SECTIONS(AROUTE_ID,ASECTIONS,AOBJECT_PATHS,ADIVIDER) AS POINT_TABLE)));
	END IF;
	...
	IF (ADATE_BEGIN IS NOT NULL) AND (ADATE_END IS NOT NULL) THEN
	  ...
	  FOR INC IN (SELECT JO.POINT_ID, JO.DATE_OBSERVATION, JO.CYCLE_ID, JO.VALUE
                   ...
                   WHERE JO.CYCLE_ID=C.CYCLE_ID
                     ...
                     AND JO.POINT_ID IN (SELECT POINT_ID FROM TABLE(CAST(GET_POINTS_IN_SECTIONS(AROUTE_ID,ASECTIONS,AOBJECT_PATHS,ADIVIDER) AS POINT_TABLE)))
