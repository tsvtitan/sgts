...
AS
  ...
  SHIFT FLOAT;
BEGIN
   SELECT JO.MEASURE_TYPE_ID, JO.CYCLE_ID, JO.WHO_CONFIRM, JO.DATE_CONFIRM,
          JO.GROUP_ID, JO.PRIORITY, JO.POINT_ID, JO.DATE_OBSERVATION, JO.PARAM_ID, C.CYCLE_NUM
     INTO AMEASURE_TYPE_ID, ACYCLE_ID, AWHO_CONFIRM, ADATE_CONFIRM,
          AGROUP_ID, APRIORITY, APOINT_ID, ADATE_OBSERVATION, APARAM_ID, ACYCLE_NUM
     FROM JOURNAL_FIELDS JO, CYCLES C
    WHERE JO.JOURNAL_FIELD_ID = CONFIRM_HND_DRAFT_BEGIN_1_2.JOURNAL_FIELD_ID
	  AND JO.CYCLE_ID=C.CYCLE_ID;
	
   IF (APARAM_ID=50008) THEN /* Отсчет (ход обратно) */
	 ...
	 IF (AROUTE_ID IS NOT NULL) THEN
	   ...
	   IF (LAST_POINT_ID=APOINT_ID) THEN
		 ...
		 IF (AWHO_CONFIRM IS NOT NULL) THEN 
		   ...
		   IF (AROUTE_ID=50028) THEN /* Галерея 1 */
		   ...
		   MIN_DATE:=NULL;
		   MIN_CYCLE_ID:=NULL;
		   FOR INC IN (SELECT JO.CYCLE_ID, JO.DATE_OBSERVATION
		   ...
		   FOR INC IN (SELECT JO.CYCLE_ID, JO.DATE_OBSERVATION
                        ...
                        WHERE JO.CYCLE_ID=C.CYCLE_ID
                          ...
                          AND JO.POINT_ID IN (R1_POINT_ID,R2_POINT_ID,R3_POINT_ID)
                          AND C.CYCLE_NUM>=ACYCLE_NUM
                       ORDER BY DATE_OBSERVATION ASC) LOOP
		   ...
		   IF (MAX_DATE-ADATE_OBSERVATION)>(ADATE_OBSERVATION-MIN_DATE) THEN
		     R_CYCLE_ID:=MIN_CYCLE_ID;
		   END IF;
		   
           CURSOR_STRI:='SELECT JOURNAL_FIELD_ID, GROUP_ID, DATE_OBSERVATION, POINT_ID, '||
          ...
          OPEN CUR FOR CURSOR_STRI						 
 	              USING F1_POINT_ID, F2_POINT_ID, R1_POINT_ID,
				        F3_POINT_ID, F4_POINT_ID, R2_POINT_ID,
						R3_POINT_ID,
						F1_POINT_ID, F2_POINT_ID, 
				        F3_POINT_ID, F4_POINT_ID, 
						F1_POINT_ID, F2_POINT_ID, 
				        F3_POINT_ID, F4_POINT_ID, 
						AROUTE_ID, AMEASURE_TYPE_ID, ACYCLE_ID, R_CYCLE_ID;

           LOOP
	         FETCH CUR INTO C_JOURNAL_FIELD_ID, C_GROUP_ID, C_DATE_OBSERVATION, C_POINT_ID, 
                            POINT_NAME, CONVERTER_NAME, EXCESS, RELATIVE, 
							FIRST_MARK, SUM_EXCESS, REC_NUM, FIRST_MARK_NEXT;
             ...
             FOR INC2 IN (SELECT CP.VALUE,
               ...
               EXIT WHEN (C_DATE_OBSERVATION>=INC2.DATE_BEGIN) AND (C_DATE_OBSERVATION<=INC2.DATE_END);
             END LOOP;
			 ...
			 SHIFT:=0;
			 IF (RESULT_MARK IS NOT NULL) AND 
			    (CROSS_MARK IS NOT NULL) AND 
				(SHIFT_BEGIN IS NOT NULL) THEN
				
			   SHIFT:=(RESULT_MARK-CROSS_MARK+(SHIFT_BEGIN/1000))*1000;
			   
			 END IF;  

			 IF (RESULT_MARK IS NOT NULL) THEN
