   ...
   THEN
      ...
      THEN
         SELECT COMPONENT_ID
           INTO COMP_ID
           FROM COMPONENTS
          WHERE (CONVERTER_ID = APOINT_ID) AND (PARAM_ID = 50003);
         ...
         LOOP
            SELECT COMPONENT_ID
              INTO COMP_ID
              FROM COMPONENTS
             WHERE (CONVERTER_ID = INC.POINT_ID) AND (PARAM_ID = 50003);
            ...
            IF (N_ZV = N_ZV_TEMP) AND (FLAG = FALSE)
            THEN
         ...
         THEN
            ...
            THEN
               ...
               SELECT POINT_ID
                 INTO R_POINT_ID
                 FROM ROUTE_POINTS
                WHERE (ROUTE_ID = AROUTE_ID) AND (PRIORITY = R_PRIORITY);
            ...
            THEN
               ...
               SELECT VALUE
                 INTO O_SR_I
                 FROM JOURNAL_OBSERVATIONS
                WHERE (GROUP_ID = AGROUP_ID) AND (PARAM_ID = 50020);
               ...
               LOOP
                  ...
                  IF (ADATE_OBSERVATION >= DATE_BEGIN) AND (DATE_END IS NULL)
                  THEN
                     SELECT VALUE
                       INTO O_SR_Z_0_1
                       FROM CONVERTER_PASSPORTS
