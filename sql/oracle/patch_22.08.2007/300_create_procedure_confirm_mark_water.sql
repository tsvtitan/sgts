...
BEGIN
  ...
  IF (APARAM_ID=2920)/* Отсчет пьезометра */ THEN 
    ...
    IF (AWHO_CONFIRM IS NOT NULL) THEN
	  ...
	  SELECT INSTRUMENT_TYPE_ID INTO AINSTRUMENT_TYPE_ID
	    FROM INSTRUMENTS WHERE INSTRUMENT_ID=AINSTRUMENT_ID;
	  
	  CASE 
	    WHEN AINSTRUMENT_TYPE_ID=2520 /* Уровнемер */ THEN
		  FLAG:=1;
		  MARK_MOUNTH:=0;
		...
		WHEN AINSTRUMENT_TYPE_ID IN (1002,2500,2502,2522,2501) /* Манометр */ THEN
		  FLAG:=1;
		  K:=1;
		  CASE AMEASURE_UNIT_ID
		    WHEN 2542 /* кг/см2 */ THEN K:=10;
		    WHEN 2610 /* МПа */ THEN K:=102;
	      END CASE;
          M_CENTER:=0;
		...
		WHEN AINSTRUMENT_TYPE_ID=2521 /* Датчик */ THEN
		  FLAG:=1;
		  MARK_SENSOR:=0;
		  FOR INC IN (SELECT CP.VALUE,
 	                         CP.DATE_BEGIN,
