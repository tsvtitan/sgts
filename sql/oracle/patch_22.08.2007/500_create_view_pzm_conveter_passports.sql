/* Создание просмотра паспортов преобразователей для Пьезометров */
...
SELECT MT.MEASURE_TYPE_ID,
  ...
  FROM CONVERTERS C, POINTS P, ROUTE_POINTS RP, ROUTES R, MEASURE_TYPE_ROUTES MTR, MEASURE_TYPES MT,
       (SELECT CONVERTER_ID,
               ...
               (SELECT MIN(CP.VALUE) FROM CONVERTER_PASSPORTS CP, COMPONENTS C 
                 WHERE CP.COMPONENT_ID=C.COMPONENT_ID
                   AND C.CONVERTER_ID=CPC.CONVERTER_ID
                   AND C.PARAM_ID=7981) AS TYPE
          FROM (SELECT C.CONVERTER_ID, CP.DATE_BEGIN, CP.DATE_END, C.PARAM_ID, 
