	                 ...
	                 (SELECT C.COMPONENT_ID, C.CONVERTER_ID 
	                   ...
	                   WHERE C.COMPONENT_ID=CP.COMPONENT_ID
			             AND C.PARAM_ID=60012 /* Дата выхода из строя */
				   		 AND ((DECODE(CP.VALUE,NULL,NULL,TO_DATE(CP.VALUE,'DD.MM.YYYY'))>=CALCULATE_NDS_ROZETKA_T_DEFORM.DATE_OBSERVATION) OR (TRIM(CP.VALUE) IS NULL))) C3									 
	              ...
	              FROM POINTS P, 
	                   ...
	                   (SELECT C.COMPONENT_ID, C.CONVERTER_ID 
	                     ...
	                     WHERE C.COMPONENT_ID=CP.COMPONENT_ID
			               AND UPPER(CP.VALUE)<>UPPER('конус')
			               AND C.PARAM_ID=60008 /* Расположение прибора */) C2,
	                   (SELECT C.COMPONENT_ID, C.CONVERTER_ID, DECODE(CP.VALUE,NULL,NULL,TO_DATE(CP.VALUE,'DD.MM.YYYY')) AS VALUE 
	                      FROM COMPONENTS C, CONVERTER_PASSPORTS CP
	                     WHERE C.COMPONENT_ID=CP.COMPONENT_ID
				           AND C.PARAM_ID=60012 /* Дата выхода из строя */
				           AND ((DECODE(CP.VALUE,NULL,NULL,TO_DATE(CP.VALUE,'DD.MM.YYYY'))>=CALCULATE_NDS_ROZETKA_T_DEFORM.DATE_OBSERVATION) OR (TRIM(CP.VALUE) IS NULL))) C3,
