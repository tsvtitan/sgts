/* Изменение процедуры утверждения всех данных */
...
AS
  ACYCLE_NUM INTEGER;
BEGIN
  IF (MEASURE_TYPE_ID IS NOT NULL) THEN

    IF (CYCLE_ID IS NOT NULL) THEN
      
	  IF (MORE IS NULL) THEN
	  
	    IF (WITH_CONFIRM IS NOT NULL) THEN
        ...
        FOR INC IN (SELECT /*+ FIRST_ROWS INDEX(JF) INDEX(MTR) INDEX(RP) INDEX(MTP) */
                           JF.JOURNAL_FIELD_ID
                      FROM JOURNAL_FIELDS JF, MEASURE_TYPE_ROUTES MTR,
                           ROUTE_POINTS RP, MEASURE_TYPE_PARAMS MTP
                     WHERE JF.MEASURE_TYPE_ID=MTR.MEASURE_TYPE_ID
                       ...
                       AND JF.IS_BASE=1
                     ORDER BY JF.DATE_OBSERVATION DESC, RP.PRIORITY DESC, MTP.PRIORITY DESC) LOOP
          CONFIRM_JOURNAL_FIELD(INC.JOURNAL_FIELD_ID);
          EXIT;
        END LOOP;
     ELSE
        SELECT CYCLE_NUM INTO ACYCLE_NUM
		  FROM CYCLES 
		...
		IF (WITH_CONFIRM IS NOT NULL) THEN 
          UPDATE JOURNAL_FIELDS
             SET WHO_CONFIRM=CONFIRM_ALL.WHO_CONFIRM,
                 DATE_CONFIRM=SYSDATE
           WHERE MEASURE_TYPE_ID=CONFIRM_ALL.MEASURE_TYPE_ID
             AND CYCLE_ID IN (SELECT C.CYCLE_ID FROM CYCLES C
