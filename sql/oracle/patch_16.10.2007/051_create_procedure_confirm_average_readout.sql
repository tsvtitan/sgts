...
CREATE OR REPLACE PROCEDURE CONFIRM_AVERAGE_READOUT (JOURNAL_FIELD_ID IN INTEGER, ALGORITHM_ID IN INTEGER)
...
NAP_XODA INTEGER;
 ...
 IF APARAM_ID=15121 AND NAP_XODA=1 /* Отсчет (ход обратно) */ THEN
 	...
 	AND PARAM_ID=17200; /* Отсчет средний */
 	COMMIT;
 	IF (AWHO_CONFIRM IS NOT NULL) THEN
	   ...
	   IF (APARAM_ID=15121) AND (NAP_XODA=1) THEN
		  ...
		  SELECT VALUE INTO L_OT FROM JOURNAL_OBSERVATIONS WHERE JOURNAL_FIELD_ID=CONFIRM_AVERAGE_READOUT.JOURNAL_FIELD_ID AND PARAM_ID=15121;
		  ...
		  I_JOURNAL_OBSERVATION(GET_JOURNAL_OBSERVATION_ID, JOURNAL_FIELD_ID, NULL, AMEASURE_TYPE_ID, 3670, ADATE_OBSERVATION, ACYCLE_ID, APOINT_ID, 17200,/* Средний отсчет */ M_OT, AWHO_CONFIRM, ADATE_CONFIRM, ALGORITHM_ID, AGROUP_ID, APRIORITY);
