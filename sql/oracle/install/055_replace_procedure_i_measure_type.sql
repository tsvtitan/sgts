/* Изменение процедуры создания вида измерения */
...
BEGIN
  ...
  SELECT COUNT(*) INTO CNT FROM GRAPHS WHERE GRAPH_ID=PARENT_ID;
  IF (CNT>0) THEN
    SELECT COUNT(*) INTO CNT FROM GRAPHS WHERE GRAPH_ID=I_MEASURE_TYPE.MEASURE_TYPE_ID;
	IF (CNT=0) THEN
      UPDATE GRAPHS
         SET GRAPH_ID=I_MEASURE_TYPE.MEASURE_TYPE_ID
       WHERE GRAPH_ID=PARENT_ID;
	END IF;
  ELSE
    SELECT COUNT(*) INTO CNT FROM GRAPHS WHERE GRAPH_ID=I_MEASURE_TYPE.MEASURE_TYPE_ID;
	IF (CNT=0) THEN
      INSERT INTO GRAPHS (GRAPH_ID) VALUES (I_MEASURE_TYPE.MEASURE_TYPE_ID);
	END IF;  
  END IF; 
