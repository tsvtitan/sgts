/* Создание процедуры установки расчет расстояний по X, Y, Z для напольных щелемеров */
...
BEGIN
  ...
  IF (DISTANCE_X IS NOT NULL) THEN
    ...
    FOR INC IN (SELECT CP.VALUE,
                 ...
                 WHERE CP.COMPONENT_ID=C.COMPONENT_ID
                   AND C.CONVERTER_ID=APOINT_ID
                   AND C.PARAM_ID=30024 /* Раскрытие с начала наблюдений по X */
                 ORDER BY CP.DATE_BEGIN) LOOP
    ...
    FOR INC IN (SELECT J.VALUE
	             ...
	             WHERE J.CYCLE_ID=C.CYCLE_ID
				   ...
				   AND J.PARAM_ID=30024 /* Раскрытие с начала наблюдений по X */
  ...
  IF (DISTANCE_Y IS NOT NULL) THEN
    ...
    FOR INC IN (SELECT CP.VALUE,
                 ...
                 WHERE CP.COMPONENT_ID=C.COMPONENT_ID
                   AND C.CONVERTER_ID=APOINT_ID
                   AND C.PARAM_ID=30025 /* Раскрытие с начала наблюдений по Y */
                 ORDER BY CP.DATE_BEGIN) LOOP
    ...
    FOR INC IN (SELECT J.VALUE
	             ...
	             WHERE J.CYCLE_ID=C.CYCLE_ID
				   AND J.POINT_ID=APOINT_ID
				   AND J.PARAM_ID=30025 /* Раскрытие с начала наблюдений по Y */
				   AND J.DATE_OBSERVATION<ADATE_OBSERVATION
			     ORDER BY J.DATE_OBSERVATION DESC) LOOP
  ...
  IF (DISTANCE_Z IS NOT NULL) THEN
    ...
    FOR INC IN (SELECT CP.VALUE,
                 ...
                 WHERE CP.COMPONENT_ID=C.COMPONENT_ID
                   AND C.CONVERTER_ID=APOINT_ID
                   AND C.PARAM_ID=30026 /* Раскрытие с начала наблюдений по Z */
                 ORDER BY CP.DATE_BEGIN) LOOP
    ...
    FOR INC IN (SELECT J.VALUE
	             ...
	             WHERE J.CYCLE_ID=C.CYCLE_ID
				   AND J.POINT_ID=APOINT_ID
				   AND J.PARAM_ID=30026 /* Раскрытие с начала наблюдений по Z */
				   AND J.DATE_OBSERVATION<ADATE_OBSERVATION
			     ORDER BY J.DATE_OBSERVATION DESC) LOOP
