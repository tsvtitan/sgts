/* Создание функции просмотра планово-высотного обоснования в полевом журнале */

CREATE OR REPLACE FUNCTION GET_PVO_JOURNAL_FIELDS 
( 
  MEASURE_TYPE_ID INTEGER, 
  IS_CLOSE INTEGER 
) 
RETURN PVO_JOURNAL_FIELD_TABLE 
PIPELINED 
IS 
  INC2 PVO_JOURNAL_FIELD_OBJECT:=PVO_JOURNAL_FIELD_OBJECT(NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL, 
                                                          NULL,NULL,NULL,NULL,NULL,NULL,NULL);   
  NUM_MIN INTEGER:=NULL; 
  NUM_MAX INTEGER:=NULL; 
BEGIN 
  FOR INC IN (SELECT MIN(CYCLE_NUM) AS NUM_MIN, MAX(CYCLE_NUM) AS NUM_MAX  
                FROM CYCLES 
               WHERE IS_CLOSE=GET_PVO_JOURNAL_FIELDS.IS_CLOSE) LOOP 
    NUM_MIN:=INC.NUM_MIN; 
 NUM_MAX:=INC.NUM_MAX; 
    EXIT;       
  END LOOP;     
  FOR INC1 IN (SELECT 
                JF.DATE_OBSERVATION,   
 JF.MEASURE_TYPE_ID, 
 MIN(P.DESCRIPTION) AS DESCRIPTION, 
 JF.NOTE, 
 JF.JOURNAL_NUM, 
               JF.CYCLE_ID, 
 MIN(JF.JOURNAL_FIELD_ID) AS JOURNAL_FIELD_ID, 
               C.CYCLE_NUM, 
               JF.POINT_ID,  
               TO_NUMBER(P.NAME) AS POINT_NAME, 
 CV.CONVERTER_ID, 
 CV.NAME AS CONVERTER_NAME, 
 JF.GROUP_ID, 
                MIN(RP.ROUTE_ID) AS ROUTE_ID, 
 MIN(DECODE(JF.PARAM_ID,50040,JF.VALUE,NULL)) AS H, 
 MIN(DECODE(JF.PARAM_ID,50043,JF.VALUE,NULL)) AS X, 
 MIN(DECODE(JF.PARAM_ID,50044,JF.VALUE,NULL)) AS Y, 
 (SELECT GO.PRIORITY AS OBJECT_PRIORITY 
           FROM GROUP_OBJECTS GO 
           WHERE GO.GROUP_ID=30060 
           AND GO.OBJECT_ID=P.OBJECT_ID) AS OBJECT_PRIORITY,        
           OT.OBJECT_PATHS 
    FROM   JOURNAL_FIELDS JF, CYCLES C, POINTS P, CONVERTERS CV,  
     ROUTE_POINTS RP, 
     (SELECT OT1.OBJECT_ID, SUBSTR(MAX(SYS_CONNECT_BY_PATH(O1.NAME,'\')),2) AS OBJECT_PATHS 
                         FROM OBJECT_TREES OT1, OBJECTS O1 
                        WHERE OT1.OBJECT_ID=O1.OBJECT_ID 
                        START WITH OT1.PARENT_ID IS NULL 
                      CONNECT BY OT1.PARENT_ID=PRIOR OT1.OBJECT_TREE_ID 
         GROUP BY OT1.OBJECT_ID) OT       
    WHERE JF.CYCLE_ID=C.CYCLE_ID 
      AND JF.POINT_ID=P.POINT_ID   
   AND OT.OBJECT_ID=P.OBJECT_ID 
   AND CV.CONVERTER_ID=P.POINT_ID 
   AND RP.POINT_ID=JF.POINT_ID 
   AND JF.MEASURE_TYPE_ID=GET_PVO_JOURNAL_FIELDS.MEASURE_TYPE_ID    
   AND JF.PARAM_ID IN (50040, 50043, 50044) 
                  AND C.CYCLE_NUM>=NUM_MIN AND C.CYCLE_NUM<=NUM_MAX 
   AND C.IS_CLOSE=GET_PVO_JOURNAL_FIELDS.IS_CLOSE 
    GROUP BY JF.DATE_OBSERVATION, JF.MEASURE_TYPE_ID, JF.CYCLE_ID, C.CYCLE_NUM, JF.POINT_ID, JF.JOURNAL_NUM, P.POINT_ID,  
  P.NAME, CV.CONVERTER_ID, CV.NAME, JF.GROUP_ID, P.COORDINATE_Z, OT.OBJECT_PATHS, RP.PRIORITY, JF.NOTE,P.OBJECT_ID 
    ORDER BY JF.DATE_OBSERVATION, JF.GROUP_ID, RP.PRIORITY)LOOP 
    INC2.CYCLE_ID:=INC1.CYCLE_ID; 
    INC2.CYCLE_NUM:=INC1.CYCLE_NUM; 
    INC2.JOURNAL_FIELD_ID:=INC1.JOURNAL_FIELD_ID;   
    INC2.JOURNAL_NUM:=INC1.JOURNAL_NUM;   
 INC2.DATE_OBSERVATION:=INC1.DATE_OBSERVATION; 
 INC2.MEASURE_TYPE_ID:=INC1.MEASURE_TYPE_ID; 
 INC2.POINT_ID:=INC1.POINT_ID; 
 INC2.POINT_NAME:=INC1.POINT_NAME; 
 INC2.ROUTE_ID:=INC1.ROUTE_ID; 
 INC2.H:=INC1.H; 
 INC2.X:=INC1.X; 
 INC2.Y:=INC1.Y; 
 INC2.CONVERTER_ID:=INC1.CONVERTER_ID; 
 INC2.CONVERTER_NAME:=INC1.CONVERTER_NAME; 
 INC2.NOTE:=INC1.NOTE; 
 INC2.DESCRIPTION:=INC1.DESCRIPTION; 
 INC2.OBJECT_PRIORITY:=INC1.OBJECT_PRIORITY; 
 INC2.OBJECT_PATHS:=INC1.OBJECT_PATHS;  
    PIPE ROW (INC2); 
  END LOOP; 
  RETURN; 
END;

--

/* Фиксация изменений */

COMMIT

