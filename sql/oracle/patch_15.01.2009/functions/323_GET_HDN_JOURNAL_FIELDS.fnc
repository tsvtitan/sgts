/* Создание функции просмотра гидронивелиров в полевом журнале */

CREATE OR REPLACE FUNCTION GET_HDN_JOURNAL_FIELDS 
( 
  MEASURE_TYPE_ID INTEGER, 
  IS_CLOSE INTEGER 
) 
RETURN HDN_JOURNAL_FIELD_TABLE 
PIPELINED 
IS 
  INC2 HDN_JOURNAL_FIELD_OBJECT:=HDN_JOURNAL_FIELD_OBJECT(NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL, 
...
BEGIN 
  FOR INC IN (SELECT MIN(CYCLE_NUM) AS NUM_MIN, MAX(CYCLE_NUM) AS NUM_MAX  
                FROM CYCLES 
               WHERE IS_CLOSE=GET_HDN_JOURNAL_FIELDS.IS_CLOSE) LOOP 
    NUM_MIN:=INC.NUM_MIN; 
 NUM_MAX:=INC.NUM_MAX; 
    EXIT;       
  END LOOP;     
  FOR INC1 IN (SELECT 
                JF.DATE_OBSERVATION, 
  MAX(JF.NOTE) AS DATE_MOTION_BACK,    
 JF.MEASURE_TYPE_ID, 
               JF.CYCLE_ID, 
 ...
 TO_NUMBER(CP.VALUE) AS HDN_NUMBER, 
       MIN(DECODE(JF.PARAM_ID,50007,JF.VALUE,NULL)) AS VALUE_MOTION_FORWARD, 
       MIN(DECODE(JF.PARAM_ID,50008,JF.VALUE,NULL)) AS VALUE_MOTION_BACK, 
    MIN(DECODE(JF.PARAM_ID,50020,JF.VALUE,NULL)) AS VALUE_AVERAGE, 
       MIN(DECODE(JF.PARAM_ID,50023,JF.VALUE,NULL)) AS VALUE_ERROR,  
  ...
  CONVERTERS CV, ROUTE_POINTS RP, ROUTES R, COMPONENTS CM, CONVERTER_PASSPORTS CP 
   ...
   AND CM.PARAM_ID = 50002 
      AND JF.MEASURE_TYPE_ID=GET_HDN_JOURNAL_FIELDS.MEASURE_TYPE_ID 
      AND JF.PARAM_ID IN  (50007, 50008, 50020, 50023) 
      AND C.CYCLE_NUM>=NUM_MIN AND C.CYCLE_NUM<=NUM_MAX 
   AND C.IS_CLOSE=GET_HDN_JOURNAL_FIELDS.IS_CLOSE 
    GROUP BY JF.DATE_OBSERVATION, JF.MEASURE_TYPE_ID, JF.CYCLE_ID, C.CYCLE_NUM, JF.POINT_ID, JF.JOURNAL_NUM, P.POINT_ID,  
  P.NAME, CV.CONVERTER_ID, CV.NAME, JF.GROUP_ID, R.NAME, O.SHORT_NAME, RP.PRIORITY, CP.VALUE 
 ...
 INC2.VALUE_ERROR:=INC1.VALUE_ERROR;   
 INC2.DATE_MOTION_BACK:=INC1.DATE_MOTION_BACK; 
