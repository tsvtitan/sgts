/* Создание функции просмотра нивелирования в полевом журнале */

CREATE OR REPLACE FUNCTION GET_NIV_JOURNAL_FIELDS 
( 
  MEASURE_TYPE_ID INTEGER, 
  IS_CLOSE INTEGER 
) 
RETURN NIV_JOURNAL_FIELD_TABLE 
PIPELINED 
IS 
  INC2 NIV_JOURNAL_FIELD_OBJECT:=NIV_JOURNAL_FIELD_OBJECT(NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL, 
...
BEGIN 
  FOR INC IN (SELECT MIN(CYCLE_NUM) AS NUM_MIN, MAX(CYCLE_NUM) AS NUM_MAX  
                FROM CYCLES 
               WHERE IS_CLOSE=GET_NIV_JOURNAL_FIELDS.IS_CLOSE) LOOP 
    NUM_MIN:=INC.NUM_MIN; 
 NUM_MAX:=INC.NUM_MAX; 
    EXIT;       
  END LOOP;     
 ...
 MIN(DECODE(JF.PARAM_ID,50040,JF.VALUE,NULL)) AS COORDINATE_Z, 
 (SELECT GO.PRIORITY AS OBJECT_PRIORITY 
   ...
   AND JF.MEASURE_TYPE_ID=GET_NIV_JOURNAL_FIELDS.MEASURE_TYPE_ID    
   AND JF.PARAM_ID =50040 
                  AND C.CYCLE_NUM>=NUM_MIN AND C.CYCLE_NUM<=NUM_MAX 
   AND C.IS_CLOSE=GET_NIV_JOURNAL_FIELDS.IS_CLOSE 
