/* Создание функции получения таблицы точек лежащих вне секций */

CREATE OR REPLACE FUNCTION GET_POINTS_OUT_SECTIONS 
( 
  ROUTE_ID IN INTEGER, 
  ...
  FOR INC IN (SELECT DISTINCT(T.POINT_ID) 
                FROM (SELECT T.POINT_ID, 
                        ...
                        FROM (SELECT RP.POINT_ID,  
                                     SUBSTR(OT.OBJECT_PATHS,LENGTH(GET_POINTS_OUT_SECTIONS.OBJECT_PATHS)+1) AS OBJECT_PATHS  
                            ...
                            WHERE RP.ROUTE_ID=GET_POINTS_OUT_SECTIONS.ROUTE_ID 
                              AND RP.POINT_ID=P.POINT_ID 
                              AND P.OBJECT_ID=OT.OBJECT_ID(+)) T) T 
                    WHERE T.OBJECT_PATHS NOT IN (SELECT VALUE FROM TABLE(CAST(GET_VARCHAR_TABLE(SECTIONS,DIVIDER) AS VARCHAR_TABLE)))) LOOP 
