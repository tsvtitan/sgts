/* Создание функции расчета критерия фильтрационного расхода через бетон напорной грани */

CREATE OR REPLACE FUNCTION CRITERIA_EXPENSE_CONCRETE 
( 
...
AS 
  ...
  AMEASURE_TYPE_ID INTEGER:=2582 /* Через бетон НГ плотины */; 
...
BEGIN 
  SELECT FIRST_MIN_VALUE, FIRST_MAX_VALUE, SECOND_MIN_VALUE, SECOND_MAX_VALUE,  
    ...
    INTO FIRST_MIN_VALUE, FIRST_MAX_VALUE, SECOND_MIN_VALUE, SECOND_MAX_VALUE,  
         FIRST_MODULO, SECOND_MODULO  
 FROM CRITERIAS  
   WHERE CRITERIA_ID=CRITERIA_EXPENSE_CONCRETE.CRITERIA_ID; 
