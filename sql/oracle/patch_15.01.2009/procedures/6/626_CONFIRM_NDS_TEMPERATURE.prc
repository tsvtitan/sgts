/* Создание процедуры расчета температуры */

CREATE OR REPLACE PROCEDURE CONFIRM_NDS_TEMPERATURE
( 
...
BEGIN 
  SELECT MEASURE_TYPE_ID,CYCLE_ID,WHO_CONFIRM,DATE_CONFIRM,GROUP_ID,PRIORITY,POINT_ID, 
    ...
    FROM JOURNAL_FIELDS 
   WHERE JOURNAL_FIELD_ID=CONFIRM_NDS_TEMPERATURE.JOURNAL_FIELD_ID; 
    
    
  IF (APARAM_ID=60002) /* Сопротивление */ AND (AVALUE<>0.0) THEN  
 
     DELETE FROM JOURNAL_OBSERVATIONS 
        WHERE JOURNAL_FIELD_ID=CONFIRM_NDS_TEMPERATURE.JOURNAL_FIELD_ID 
             AND ALGORITHM_ID=CONFIRM_NDS_TEMPERATURE.ALGORITHM_ID  
             ...
             AND PARAM_ID IN (60036); /* Температура бетона */ 
    COMMIT; 
  ...
  IF (UPPER(TYPE_INSTRUMENT) IN (UPPER('телетензометр'),UPPER('телетермометр'), 
                                 UPPER('щелемер дистанционный'),UPPER('телещелемер'))) THEN 
...
/*      DBMS_OUTPUT.PUT_LINE('==========================================================');    
    ...
    CALCULATE_NDS_TEMPERATURE(APOINT_ID,AVALUE,RESISTANCE_LINE, 
