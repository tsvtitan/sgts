/* Создание процедуры утверждения CONFIRM_DISPLACEMENT_SYMBOL_X */

CREATE OR REPLACE PROCEDURE CONFIRM_DISPLACEMENT_SYMBOL_X
( 
...
BEGIN 
  SELECT MEASURE_TYPE_ID,CYCLE_ID,WHO_CONFIRM,DATE_CONFIRM,GROUP_ID,PRIORITY,POINT_ID, 
    ...
    FROM JOURNAL_FIELDS 
   WHERE JOURNAL_FIELD_ID=CONFIRM_DISPLACEMENT_SYMBOL_X.JOURNAL_FIELD_ID; 
 
  IF (APARAM_ID=50043)THEN 
 
     DELETE FROM JOURNAL_OBSERVATIONS 
        WHERE JOURNAL_FIELD_ID=CONFIRM_DISPLACEMENT_SYMBOL_X.JOURNAL_FIELD_ID 
             AND ALGORITHM_ID=CONFIRM_DISPLACEMENT_SYMBOL_X.ALGORITHM_ID 
    ...
    IF (AWHO_CONFIRM IS NOT NULL) THEN 
     FOR INC IN (SELECT COORDINATE_X 
                 FROM POINTS 
                 WHERE POINT_ID=APOINT_ID) LOOP 
      DX:=(X-INC.COORDINATE_X)*1000; 
   EXIT; 
    END LOOP;   
