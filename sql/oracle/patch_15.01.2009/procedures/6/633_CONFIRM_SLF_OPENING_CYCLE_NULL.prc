/* Создание процедуры расчета раскрытия напольных щелемеров с нулевого цикла */

CREATE OR REPLACE PROCEDURE CONFIRM_SLF_OPENING_CYCLE_NULL (
...
BEGIN
   SELECT MEASURE_TYPE_ID,
     ...
     INTO AMEASURE_TYPE_ID,
          ...
          APARAM_ID
     FROM JOURNAL_FIELDS
    WHERE JOURNAL_FIELD_ID = CONFIRM_SLF_OPENING_CYCLE_NULL.JOURNAL_FIELD_ID;

   IF (APARAM_ID IN (30024, 30025, 30026)) /* Раскрытие с начала наблюдений по X, Y, Z */
   THEN
      IF (APARAM_ID = 30024)
      THEN
         DPARAM_ID := 30072;
      ...
      DELETE FROM JOURNAL_OBSERVATIONS
            WHERE JOURNAL_FIELD_ID = CONFIRM_SLF_OPENING_CYCLE_NULL.JOURNAL_FIELD_ID
              AND ALGORITHM_ID = CONFIRM_SLF_OPENING_CYCLE_NULL.ALGORITHM_ID
              ...
              AND PARAM_ID = DPARAM_ID;

      /* Раскрытие с нулевого цикла по X, Y, Z*/
      COMMIT;
