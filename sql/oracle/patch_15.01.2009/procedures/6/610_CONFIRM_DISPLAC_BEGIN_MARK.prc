...
BEGIN
   ...
   THEN
      ...
      /* Смещение марки с начала наблюдений */
      COMMIT;

      IF (AWHO_CONFIRM IS NOT NULL)
      THEN /* Обработка */                         /* Находим значения отсчета(ход прямо), отсчета(ход обратно) и место нуля */
         SELECT VALUE
           INTO L_OT
         ...
         I_JOURNAL_OBSERVATION (GET_JOURNAL_OBSERVATION_ID,
                                ...
                                ACYCLE_ID,
                                APOINT_ID,
                                17181, /* Смещение марки с начала наблюдений */
                                DISPLACEMENT,
                                AWHO_CONFIRM,
                                ADATE_CONFIRM,
                                ALGORITHM_ID,
