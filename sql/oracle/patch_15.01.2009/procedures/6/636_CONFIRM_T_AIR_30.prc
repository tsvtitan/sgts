/* Создание процедуры утверждения и расчета температуры воздуха за 30 суток */

CREATE OR REPLACE PROCEDURE CONFIRM_T_AIR_30
(
...
BEGIN
  SELECT MEASURE_TYPE_ID,CYCLE_ID,WHO_CONFIRM,DATE_CONFIRM,GROUP_ID,PRIORITY,
    ...
    FROM JOURNAL_FIELDS
   WHERE JOURNAL_FIELD_ID=CONFIRM_T_AIR_30.JOURNAL_FIELD_ID;

  IF (APARAM_ID=2903) /* Температура воздуха */ THEN

    DELETE FROM JOURNAL_OBSERVATIONS
          WHERE JOURNAL_FIELD_ID=CONFIRM_T_AIR_30.JOURNAL_FIELD_ID
            AND ALGORITHM_ID=CONFIRM_T_AIR_30.ALGORITHM_ID 
            ...
            AND PARAM_ID=2913;/* Температура воздуха осредненная за 30 суток */
    COMMIT;
  
    IF (AWHO_CONFIRM IS NOT NULL) THEN
  
   SELECT SUM(VALUE)/30 INTO T_AIR_30 
     FROM JOURNAL_FIELDS
    WHERE MEASURE_TYPE_ID=AMEASURE_TYPE_ID
      AND PARAM_ID=2903 /* Температура воздуха */
      AND DATE_OBSERVATION>(ADATE_OBSERVATION-30)
      AND DATE_OBSERVATION<=ADATE_OBSERVATION;
    
   IF (T_AIR_30 IS NOT NULL) THEN      
        I_JOURNAL_OBSERVATION(GET_JOURNAL_OBSERVATION_ID,
              ...
              ACYCLE_ID,
              APOINT_ID,
              2913,/* Температура воздуха осредненная за 30 суток */
              T_AIR_30,
              AWHO_CONFIRM,
              ADATE_CONFIRM,
              ALGORITHM_ID,
