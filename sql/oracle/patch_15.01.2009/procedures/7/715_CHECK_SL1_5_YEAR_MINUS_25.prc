/* Создание процедуры проверки значения раскрытия одноосных щелемеров за 5 лет -25% */

CREATE OR REPLACE PROCEDURE CHECK_SL1_5_YEAR_MINUS_25 (
...
BEGIN
   ...
   SELECT SUM (JF.VALUE) / COUNT (*)
    ...
    WHERE EXTRACT (MONTH FROM JF.DATE_OBSERVATION) = AMONTH
      ...
      AND JF.MEASURE_TYPE_ID = CHECK_SL1_5_YEAR_MINUS_25.MEASURE_TYPE_ID
      AND JF.POINT_ID = CHECK_SL1_5_YEAR_MINUS_25.POINT_ID
      AND JF.PARAM_ID = 30002   /* Раскрытие с начала наблюдений */
      AND JF.DATE_CONFIRM IS NOT NULL;

   AVALUE := AVALUE - AVALUE * (PERC / 100);
   S := TO_CHAR (AVALUE);

   IF (OPENING < AVALUE)
   THEN
      SUCCESS := 0;
      INFO := 'Контроль не пройден. Раскрытие=' || TO_CHAR (OPENING) || ' и < ' || S;
   ELSE
      SUCCESS := 1;
      INFO := 'Контроль пройден. Раскрытие=' || TO_CHAR (OPENING) || ' и >= ' || S;
   END IF;
END;
