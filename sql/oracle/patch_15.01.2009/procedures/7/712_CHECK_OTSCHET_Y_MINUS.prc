/* Создание процедуры контроля CHECK_OTSCHET_Y_MINUS */

CREATE OR REPLACE PROCEDURE CHECK_OTSCHET_Y_MINUS (
...
BEGIN
   DIS_BEG := GET_DISPLACEMENT_BEGIN_Y (CHECK_OTSCHET_Y_MINUS.DATE_OBSERVATION, CHECK_OTSCHET_Y_MINUS.POINT_ID, VALUE);
   CUR_DIS := GET_CURRENT_DISPLAC_Y (CHECK_OTSCHET_Y_MINUS.DATE_OBSERVATION, CHECK_OTSCHET_Y_MINUS.POINT_ID, DIS_BEG);
   ...
   SELECT SUM (JF.VALUE) / COUNT (*)
    ...
    WHERE EXTRACT (MONTH FROM JF.DATE_OBSERVATION) = AMONTH
      ...
      AND JF.MEASURE_TYPE_ID = CHECK_OTSCHET_Y_MINUS.MEASURE_TYPE_ID
      AND JF.POINT_ID = CHECK_OTSCHET_Y_MINUS.POINT_ID
      AND JF.PARAM_ID = 17162
