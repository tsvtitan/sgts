...
CREATE OR REPLACE PROCEDURE CHECK_SL1_5_YEAR_PLUS_25 (
...
BEGIN
   ...
   SELECT SUM (JF.VALUE) / COUNT (*)
    ...
    WHERE EXTRACT (MONTH FROM JF.DATE_OBSERVATION) = AMONTH
      ...
      AND JF.MEASURE_TYPE_ID = CHECK_SL1_5_YEAR_PLUS_25.MEASURE_TYPE_ID
      AND JF.POINT_ID = CHECK_SL1_5_YEAR_PLUS_25.POINT_ID
      AND JF.PARAM_ID = 30002   /* Раскрытие с начала наблюдений */
      AND JF.DATE_CONFIRM IS NOT NULL;

   AVALUE := AVALUE + AVALUE * 0.25;
   S := TO_CHAR (AVALUE);
