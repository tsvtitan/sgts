/* Создание процедуры контроля CHECK_OTSCHET_X_PLUS */

CREATE OR REPLACE PROCEDURE CHECK_OTSCHET_X_PLUS (
...
BEGIN
   DIS_BEG := GET_DISPLACEMENT_BEGIN_X (CHECK_OTSCHET_X_PLUS.DATE_OBSERVATION, CHECK_OTSCHET_X_PLUS.POINT_ID, VALUE);
   CUR_DIS := GET_CURRENT_DISPLAC_X (CHECK_OTSCHET_X_PLUS.DATE_OBSERVATION, CHECK_OTSCHET_X_PLUS.POINT_ID, DIS_BEG);
   ...
   SELECT SUM (JF.VALUE) / COUNT (*)
    ...
    WHERE EXTRACT (MONTH FROM JF.DATE_OBSERVATION) = AMONTH
      ...
      AND JF.MEASURE_TYPE_ID = CHECK_OTSCHET_X_PLUS.MEASURE_TYPE_ID
      AND JF.POINT_ID = CHECK_OTSCHET_X_PLUS.POINT_ID
