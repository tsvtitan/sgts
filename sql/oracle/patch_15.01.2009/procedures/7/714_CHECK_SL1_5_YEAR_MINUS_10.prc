/* Создание процедуры проверки значения раскрытия одноосных щелемеров за 5 лет -10% */

CREATE OR REPLACE PROCEDURE CHECK_SL1_5_YEAR_MINUS_10 (
...
BEGIN
   ...
   SELECT SUM (JF.VALUE) / COUNT (*)
    ...
    WHERE EXTRACT (MONTH FROM JF.DATE_OBSERVATION) = AMONTH
      ...
      AND JF.MEASURE_TYPE_ID = CHECK_SL1_5_YEAR_MINUS_10.MEASURE_TYPE_ID
      AND JF.POINT_ID = CHECK_SL1_5_YEAR_MINUS_10.POINT_ID
