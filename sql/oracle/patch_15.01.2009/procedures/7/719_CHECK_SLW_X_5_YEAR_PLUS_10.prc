/* Создание процедуры проверки значения раскрытия по X настенных щелемеров за 5 лет +10% */

CREATE OR REPLACE PROCEDURE CHECK_SLW_X_5_YEAR_PLUS_10 (
...
BEGIN
   ...
   SELECT SUM (JF.VALUE) / COUNT (*)
    ...
    WHERE EXTRACT (MONTH FROM JF.DATE_OBSERVATION) = AMONTH
      ...
      AND JF.MEASURE_TYPE_ID = CHECK_SLW_X_5_YEAR_PLUS_10.MEASURE_TYPE_ID
      AND JF.POINT_ID = CHECK_SLW_X_5_YEAR_PLUS_10.POINT_ID
      AND JF.PARAM_ID = 30010 /* Раскрытие с начала наблюдений по X */
   ...
   THEN
      SUCCESS := 0;
      INFO := 'Контроль не пройден. Раскрытие по X=' || TO_CHAR (OPENING) || ' и > ' || S;
   ELSE
      SUCCESS := 1;
      INFO := 'Контроль пройден. Раскрытие по X=' || TO_CHAR (OPENING) || ' и <= ' || S;
   END IF;
END;
